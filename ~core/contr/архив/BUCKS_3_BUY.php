...
//=================================================================================================
        ...
        // $symbol['klinesPrice'] = '';
               ...
               $interval = '1m';        
                ...
                if (!$klin = $Bin->klines(array('symbol'=>$value['symbol'], 'interval' => $interval, 'limit' => 1000))) {
                  $symbol['statusTREND'] = 'NOT klines';
                  continue;
                }
                // $Bin->showArrayTable($klin);
                
                $fileklines = 'E:\binance\symbols\\'.$value['symbol'].'_klines_'.$interval.'_funded.txt';
                ...
                $symbol['klines'] = count($klines); 
                  ...
                  $settings['quote_volume'] = 60798; 
                  $settings['TakeProfit'] = 1.015;                  
                ...
                foreach ($strateg as $key1 => $str) { 
                  $temp = [];
                  $temp['strateg'] = $str;
                  $strateg_values = array_values($Bin->multiSearch($test, array('strateg'=>$str)));
                  // $Bin->showArrayTable($strateg_values);
                  foreach ($variant as $key2 => $var) {
                      $values = array_values($Bin->multiSearch($strateg_values, array('status' => $var)));
                      $temp[$var] = count($values);
                      $temp[$var.'_total'] = array_sum(array_column($values, 'profit(loss)')); 

                  }
                  $temp['max_open'] = max(array_column($strateg_values, 'open'));
                  $temp['profit'] = $temp['TP_total'] + $temp['SL_total'] + $temp['out_total'];
                  // $Bin->show($symbol);
                  $strat = $symbol; 
                  $strat += $temp;
                ...
                // $TP = array_values($Bin->multiSearch($test, array('status' => 'TP')));
                // $symbol['TP'] = count($TP);        

                // //выбераем SL
                // $SL = array_values($Bin->multiSearch($test, array('status' => 'SL')));
                // $symbol['SL'] = count($SL);   

                // //выбераем out
                // $out = array_values($Bin->multiSearch($test, array('status' => 'out')));
                // $symbol['out'] = count($out); 
                
                // $symbol['BALANCE']= round(array_sum(array_column($test, 'profit(loss)')), 3);;
                 
                // $Bin->showArrayTable($test);
                $Bin->showArrayTable($klines);




                  unset($key_BUY, $klin, $klines);
